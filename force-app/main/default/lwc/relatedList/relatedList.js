    ...
    createColumns(){
        ...
        if (this.records && this.records.info){
            ...
            columns = this.records.info.displayColumns.map(displayColumn => {
                const fieldDescribe = this.fieldDescribes[displayColumn.fieldApiName] || {};
                let dataType = DESCRIBE_TO_DATA_TABLE_MAP[fieldDescribe.dataType];
                if (!dataType) {
                    dataType = 'text';
                }
                return {
